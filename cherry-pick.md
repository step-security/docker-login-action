...
## Context
PR: https://github.com/step-security/docker-login-action/pull/50
...
- **Workflow file changes** — workflow files are never applied by the script, but they **are** listed in the report under *"Workflow Files (Cannot be auto-applied by GitHub Actions)"*. Walk through that list and, for each file that still exists in our repo, apply the upstream v(prev)…v(target) delta manually. The categories of change to bring over:
  ...
  - **New top-level `permissions:` blocks** that upstream adds for least-privilege (e.g. `permissions: contents: read`).
...
1. `yarn install` (or `npm install`) — regenerates the lockfile to match any `package.json` changes.
2. `yarn build` — regenerates `dist/` with the new toolchain. Confirm `dist/` ends up with the same file set as upstream (extra build artifacts from the old toolchain must be removed by hand).
3. `yarn test` — runs the test suite to confirm the new bundle and dep bumps work.
